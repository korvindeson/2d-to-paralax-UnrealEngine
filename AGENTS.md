...
- **Phase H design contract**: `FaceParallaxLayoutSpec.h` (root, pure C++17, synced into the editor module Private dir) is the layout manifest + P1–P23 validator for the widget tree. `RebuildWidget` self-checks it; `Tests\ParallaxMathTests.cpp::TestPhaseHUIDesign` asserts zero violations, the exact 6-rail scroll-viewport set (rails 180×560 clipped stacks — P22: rails never scroll horizontally, every non-flex child must fit the `FixedW` margin box, and `bAspectRatio` nodes must resolve to `FaceAspectRatio` (P23, `CN-Preview` 450×450); `PR-Scroll` mirroring the props pane's `PropScroll`), mirrored design constants (P14: props pane keeps `PropsRightGap` from the window edge; P15: `PR-Scroll` keeps `PropsScrollInsetR` before its scrollbar), negative controls for every principle, and section auto-stack (manifest sections are never Flex in a clipped viewport and sibling rects never overlap). The Python validator additionally flags any real-widget section slot (`->AddSlot()` feeding a `MakeSectionBox`) that lacks `.AutoHeight()`/`.FillHeight()` — the "View Override paints over Scale Y" defect class. Pool order is child-first (args evaluate eagerly) — never index a child via `parent + N` arithmetic; use `Children[...]` lookups (`Bod(sec) = Children[1]`).

## Phase status (all integrated + verified)

Every phase is landed in the product code AND covered by a live suite in `Tests\ParallaxMathTests.cpp` (all green: 1764/1764 + validator + UE build + headless probe):
...
- **Phase H** (layout design contract, P1–P23 validator, self-check in `RebuildWidget`) — `TestPhaseHUIDesign`.
