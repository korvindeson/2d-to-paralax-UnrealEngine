...
- **Plugin**: `SAMPLES\MyProject\Plugins\FaceParallax\` — two modules:
  ...
  - `FaceParallaxEditor` (Editor, `PostDefault`) — editor subsystem, editor widget, deploy pipeline. The widget is split across `FaceParallaxEditorWidget.cpp` (core API), `FaceParallaxEditorWidgetUI.cpp` (`RebuildWidget`), `FaceParallaxEditorWidgetInteractions.cpp`, `FaceParallaxEditorWidgetPanels.cpp`, plus `FaceParallaxEditorWidgetShared.h` (anonymous-namespace helpers + `SFaceLayerGizmo` used by all widget TUs).
