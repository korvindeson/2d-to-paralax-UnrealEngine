...
editor tool needs:
  ...
  5. WBP_FaceParallaxEditor Widget Blueprint parented to the C++
   ...
   7. The VECTOR ART LIBRARY: the 17 per-feature grid SVGs in Art\_grids/
      (generated by generate_art.py) are imported into UDa_FaceVectorArt
      data assets (DA_FaceVectorArt_<Feature>) and wired onto the preset's
      LibraryVectorArt map, verified zero-gap against Art\_tokens.json.
...
# Must match EFaceAngleState enum names exactly (WI1: 14 states — the
# Narrow/Sliver sub-threshold views sit between Front/3/4 and 3/4/Profile).
VIEW_STATES = [
    "Front", "NarrowRight", "ThreeQuarterRight", "SliverRight", "RightProfile",
    "BackRight", "Back", "BackLeft", "LeftProfile", "SliverLeft",
    "ThreeQuarterLeft", "NarrowLeft", "Top", "Bottom",
]
...
# Vector art library (guide-token grid files generated by generate_art.py).
VECTOR_ART_CLASS_NAME = "FaceVectorArt"
VECTOR_ART_OUTPUT_PATH = "/Game/FaceParallax/VectorArt"

# Cell-encoding schema for the vector art assets (mirrors
# UFaceVectorArt::CurrentVectorArtSchema). Bump together with any change to
# the path encoding (FFaceVectorCmdPt/ConvertPath/ImportFromGridSvg) so the
# freshness gate below re-imports stale assets instead of trusting their
# cell-key set.
VECTOR_ART_SCHEMA = 1

# Set True to force a full re-import of every grid even when keys match —
# the documented remediation for "stale cells on disk" (rule 15: never
# delete assets; re-import in place instead).
...
REPO_ROOT = PLUGIN_SOURCE_ROOT

# Vector art library on disk (generated by generate_art.py in the repo root).
ART_ROOT = os.path.join(PLUGIN_SOURCE_ROOT, "Art")
...
PLUGIN_SOURCE_DIRS = [
    ("Source/FaceParallax/Public", [
        ...
        "FaceParallaxSchematic.h", "FaceParallaxSvgParse.h", "FaceParallaxVectorArt.h",
    ]),
    ("Source/FaceParallax/Private", [
        ...
        "FaceParallaxModule.cpp", "FaceParallaxVectorArt.cpp",
    ]),
...
# --------------------------------------------------------------
# 3.5. Vector Art Library (per-feature grid assets + preset wiring)
# --------------------------------------------------------------
def _load_tokens_manifest():
    """Read Art/_tokens.json (written by generate_art.py). None on failure."""
    if not os.path.isfile(TOKENS_JSON):
        unreal.log_warning(f"[VECTOR] manifest missing: {TOKENS_JSON} (run py generate_art.py first)")
        return None
    try:
        with open(TOKENS_JSON, encoding="utf-8") as f:
            return json.load(f)
    except Exception as e:
        unreal.log_warning(f"[VECTOR] could not read {TOKENS_JSON}: {e}")
        return None


def _vector_asset_path(feature):
    return f"{VECTOR_ART_OUTPUT_PATH}/DA_FaceVectorArt_{feature}"


def _vector_asset_valid(asset):
    """True when the loaded object is a FaceVectorArt data asset."""
    try:
        return asset is not None and asset.get_class().get_name() == "FaceVectorArt"
    except Exception:
        return False


def _vector_cell_keys(asset):
    """Read the asset's cell keys through the C++ UFUNCTION."""
    try:
        keys = asset.get_cell_keys()
        return set(keys) if keys else set()
    except Exception:
        return set()


def _vector_asset_schema(asset):
    """The ArtSchemaVersion stamped on the asset (0 = pre-schema assets)."""
    try:
        return int(asset.get_editor_property("ArtSchemaVersion"))
    except Exception:
        return 0


def _vector_cell_stats(asset, cell_key):
    """(path count, cubic count) for one cell; (0, 0) when missing/empty."""
    try:
        n_paths, n_cubics = asset.get_cell_stats(cell_key)
        return int(n_paths), int(n_cubics)
    except Exception:
        return 0, 0


def _vector_content_probe(asset, manifest_keys, feature):
    """Per-cell non-emptiness + curve-presence probe (guide IX.5 content gate,
    not just key-set): every manifest cell must hold at least one path, and a
    sampled subset (first 5 keys + the first _L key) must contain CubicTo runs
    — the flattened-MoveTo/LineTo stale-encoding regression fails this.
    Returns True when the asset content is healthy."""
    ok = True
    empty = []
    for key in sorted(manifest_keys):
        n_paths, n_cubics = _vector_cell_stats(asset, key)
        if n_paths < 1:
            empty.append(key)
            ok = False
    if empty:
        unreal.log_error(
            f"[VERIFY] {feature}: {len(empty)} manifest cell(s) parsed to zero "
            f"paths: {', '.join(sorted(empty)[:5])}{' ...' if len(empty) > 5 else ''}")
    sampled = sorted(manifest_keys)[:5]
    lefts = [k for k in manifest_keys if k.endswith("_L")]
    if lefts:
        sampled.append(sorted(lefts)[0])
    flat = [k for k in sampled if _vector_cell_stats(asset, k)[1] < 1]
    if flat:
        unreal.log_error(
            f"[VERIFY] {feature}: curve probe FAILED — cells carry no CubicTo "
            f"runs (flattened/stale encoding?): {', '.join(sorted(flat))}")
        ok = False
    return ok


def _count_paths_in_svg(text):
    return text.count("<path")


def _grid_drift_check(feature, manifest_entry, grid_text):
    """Grid-vs-individual drift (guide IX.5): the grid is the sole deploy
    source — if a cell's path count in the grid disagrees with its individual
    sibling file, the grid drifted and deploy would ship the wrong content.
    Samples the first 3 file entries that exist; returns True when healthy."""
    ...
    for fname in files:
        ...
        if not fname.endswith(".svg"):
            continue
        indiv = os.path.join(ART_ROOT, feature, fname)
        if not os.path.isfile(indiv):
            continue
        cell_key = fname[:-4]
        m = re.search(r'<g id="' + re.escape(cell_key) + r'"[^>]*>(.*?)</g>',
                      grid_text, re.DOTALL)
        if not m:
            continue
        grid_paths = _count_paths_in_svg(m.group(1))
        with open(indiv, encoding="utf-8") as f:
            indiv_paths = _count_paths_in_svg(f.read())
        if grid_paths != indiv_paths:
            unreal.log_error(
                f"[VERIFY] {feature}: drift — grid cell {cell_key} has "
                f"{grid_paths} paths but the individual file has {indiv_paths}")
            ok = False
        sampled += 1
    return ok


def _manifest_fresh_check():
    """Warn when _tokens.json is older than the schematic header it was
    generated from — a stale manifest provisions stale grids with a vacuous
    zero-gap pass (guide IX.3)."""
    try:
        header = os.path.join(REPO_ROOT, "FaceParallaxSchematic.h")
        if not os.path.isfile(header):
            return
        if os.path.getmtime(TOKENS_JSON) < os.path.getmtime(header):
            unreal.log_warning(
                "[VECTOR] Art/_tokens.json is OLDER than FaceParallaxSchematic.h "
                "- run `py generate_art.py` before deploying (the manifest "
                "provisions STALE grids otherwise)")
    except Exception:
        pass


def create_vector_art_library(preset):
    """Import the 17 per-feature grid SVGs into UDa_FaceVectorArt assets and
    wire them onto the preset's LibraryVectorArt map.

    Idempotent: an existing asset whose cell-key set exactly matches the
    manifest is skipped. Any mismatch (stale art on disk, partial import,
    wrong class) re-imports in place. Ends with a zero-gap verification —
    every manifest cell key must exist in the matching asset and vice versa.
    """
    manifest = _load_tokens_manifest()
    if manifest is None:
        return False
    _manifest_fresh_check()
    features = manifest.get("features")
    if not features or len(features) != 17:
        unreal.log_warning(f"[VECTOR] manifest has {len(features) if features else 0} features, expected 17")
        return False

    ensure_dir(VECTOR_ART_OUTPUT_PATH)
    vector_class = find_class(VECTOR_ART_CLASS_NAME)

    ok = True
    wired = 0
    for feature in sorted(features.keys()):
        grid_path = os.path.join(GRID_ROOT, f"{feature}.svg")
        if not os.path.isfile(grid_path):
            unreal.log_error(f"[VECTOR] grid file missing: {grid_path}")
            ok = False
            continue
        try:
            ...
            unreal.log_error(f"[VECTOR] could not read {grid_path}: {e}")
            ok = False
            continue

        manifest_keys = set(features[feature].get("cell_keys", []))
        if not manifest_keys:
            manifest_keys = set(k[:-4] for k in features[feature].get("files", [])
                                if k.endswith(".svg"))

        asset_path = _vector_asset_path(feature)
        asset = None
        need_import = True
        if editor_asset_lib.does_asset_exist(asset_path):
            existing = unreal.load_asset(asset_path)
            if _vector_asset_valid(existing):
                asset = existing
                existing_keys = _vector_cell_keys(existing)
                schema = _vector_asset_schema(existing)
                if FORCE_REIMPORT:
                    unreal.log(
                        f"[VECTOR] {feature}: FORCE_REIMPORT - re-importing")
                elif existing_keys != manifest_keys:
                    unreal.log(
                        f"[VECTOR] {feature}: cell keys differ from manifest "
                        f"({len(existing_keys)} vs {len(manifest_keys)}) - re-importing")
                elif schema != VECTOR_ART_SCHEMA:
                    unreal.log(
                        f"[VECTOR] {feature}: art schema v{schema} != "
                        f"v{VECTOR_ART_SCHEMA} (parser/encoding changed) - re-importing")
                elif not _vector_content_probe(existing, manifest_keys, feature):
                    unreal.log(
                        f"[VECTOR] {feature}: content probe failed (empty or "
                        f"flattened cells) - re-importing")
                else:
                    need_import = False
            else:
                unreal.log_warning(
                    f"[REPAIR] {asset_path} exists but is not a FaceVectorArt "
                    f"({existing.get_class().get_name() if existing else 'None'}) - recreating")
                editor_asset_lib.delete_asset(asset_path)
                wait_asset_gone(asset_path)

        if need_import:
            if asset is None:
                factory = unreal.DataAssetFactory()
                factory.set_editor_property("data_asset_class", vector_class)
                asset = asset_tools.create_asset(
                    f"DA_FaceVectorArt_{feature}", VECTOR_ART_OUTPUT_PATH, vector_class, factory)
                if not asset:
                    unreal.log_error(f"[VECTOR] could not create asset for {feature}")
                    ok = False
                    continue
            imported = False
            try:
                imported = asset.import_from_grid_svg(svg_text)
            except Exception as e:
                unreal.log_error(f"[VECTOR] import_from_grid_svg raised for {feature}: {e}")
            if not imported:
                try:
                    err = asset.get_editor_property("LastImportError")
                except Exception:
                    err = "(unreadable)"
                unreal.log_error(f"[VECTOR] import failed for {feature}: {err}")
                ok = False
                continue
            try:
                asset.set_editor_property("FeatureToken", feature)
            except Exception:
                pass
            editor_asset_lib.save_loaded_asset(asset)
            unreal.log(f"[OK] Imported {feature} grid -> {asset_path}")

        if preset is not None:
            try:
                preset.set_library_vector_art(feature, asset)
                wired += 1
            except Exception as e:
                unreal.log_warning(f"[VECTOR] could not wire {feature} onto preset: {e}")
                ok = False

        # Zero-gap check per feature: asset cell set == manifest cell set AND
        # the content probe passes (per-cell non-emptiness + curve presence).
        if _vector_cell_keys(asset) != manifest_keys:
            unreal.log_error(
                f"[VERIFY] {feature}: asset cells != manifest after import "
                f"(gap check failed)")
            ok = False
        if not _vector_content_probe(asset, manifest_keys, feature):
            ok = False
        if not _grid_drift_check(feature, features[feature], svg_text):
            ok = False

    if preset is not None:
        unreal.log(f"[VECTOR] Wired {wired}/17 library assets onto preset LibraryVectorArt")
        # Guide Part 0 "single window for the art": deploying the library
        # turns the 3D preview's vector-albedo mode ON by default (the
        # Preview & Debug Camera checkbox stays as the fallback switch).
        try:
            preset.set_editor_property("bUseVectorArtAlbedo", True)
            unreal.log("[VECTOR] bUseVectorArtAlbedo enabled on the preset (vector albedo default-on)")
        except Exception:
            unreal.log_warning("[VECTOR] could not enable bUseVectorArtAlbedo on the preset")
    else:
        unreal.log_warning("[VECTOR] No preset available - library assets created but not wired")

    if ok:
        unreal.log(f"[VECTOR] Vector art library complete (17/17 features, "
                   f"{manifest.get('total_cells', 0)} cells per manifest)")
    return ok
...
def verify_deployment():
    ...
    manifest = _load_tokens_manifest()
    if manifest and manifest.get("features"):
        for feature in sorted(manifest["features"].keys()):
            checks.append((f"VectorArt {feature}", _vector_asset_path(feature)))
    else:
        unreal.log_warning("[VERIFY] Vector manifest missing - vector library assets not verified")
    missing = []
...
def main():
    ...
    required = [PRESET_CLASS_NAME, COMPONENT_CLASS_NAME, "FaceParallaxEditorWidget",
                "FaceParallaxPreviewActor", VECTOR_ART_CLASS_NAME]
    ...
    if not missing_classes:
        ...
        try:
            if not create_vector_art_library(preset):
                step_fail("vector art library import incomplete")
        except Exception as e:
            step_fail(f"vector art library phase raised: {e}")
        try:
    ...
    if failures == 0 and deployed_ok:
        ...
        unreal.log(f"   - Vector Art Library: {VECTOR_ART_OUTPUT_PATH}/DA_FaceVectorArt_<Feature> (17 assets)")
