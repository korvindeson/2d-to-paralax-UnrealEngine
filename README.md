...
- **3-pane workspace (Phase A)** — left icon rail (Layers | Transform | Camera | Debug | Advanced | Assign) + `SWidgetSwitcher` rail panels, center preview canvas (display-mode row + overlay stack), right selected-slot properties pane. Old T0–T3 tabs were re-homed into the rails (rail-local Quick Actions/Sync/Alignment→Transform; Camera Follow/zones/blend→Camera; import/config/outline→depth→Debug; cross-layer/params/nested→Advanced)
...
- **Display-mode dedupe (Phase P3)** — the canvas mode row is now derived from the Debug rail's three toggles: `RefreshUI` re-runs `DeriveDisplayMode` (0 Textured, 1 Depth, 2 Wireframe, 3 Split; −1 custom combo / all-off clears the highlight), and the mode buttons' highlights are live lambdas instead of stale build-time colors
...
| `SetActiveRailIndex(Index)` | Switches the left rail panel (0 Layers, 1 Transform, 2 Camera, 3 Debug, 4 Advanced, 5 Assign) |
