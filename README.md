...
| `Tests/ParallaxMathTests.cpp` | Math tests | Standalone C++17 (no UE dep) — 2013 tests covering state machine, transforms, blink/expression/viseme, swoosh, parameters, nested art + jiggle, 3D pin projection + view-angle rotation, batch ops, zone multipliers, per-view visual hull, Phase B–H mirror suites + Phase H layout-design contract, part schematic + yaw rules + coverage/aliases + hair system + canvas filters + midpoint jiggle ramp + edge-map group mirrors, the hybrid smooth-turn orientation contract (`TestPhase2Orientation`), and the up/down pitch scrub contract (`TestPhaseCUpDownScrub`) |
