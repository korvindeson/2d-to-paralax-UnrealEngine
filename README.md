...
| `Tests/ParallaxMathTests.cpp` | Math tests | Standalone C++17 (no UE dep) — 1791 tests covering state machine, transforms, blink/expression/viseme, swoosh, parameters, nested art + jiggle, 3D pin projection + view-angle rotation, batch ops, zone multipliers, per-view visual hull, Phase B–H mirror suites + Phase H layout-design contract, part schematic + yaw rules + coverage/aliases + hair system + canvas filters + midpoint jiggle ramp + edge-map group mirrors |
...
Tests/
  ParallaxMathTests.cpp                  — 1791 standalone C++ tests
