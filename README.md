...
| `Tests/ParallaxMathTests.cpp` | Math tests | Standalone C++17 (no UE dep) — 1716 tests covering state machine, transforms, blink/expression/viseme, swoosh, parameters, nested art + jiggle, 3D pin projection + view-angle rotation, batch ops, zone multipliers, per-view visual hull, Phase H layout-design contract, part schematic + yaw rules + coverage/aliases + hair system + canvas filters + midpoint jiggle ramp |
...
- **Undo/redo (real stack)** — every preset mutation wraps in `FWidgetUndoScope`, which pushes a pre-mutation preset duplicate onto the 32-entry `UndoStack` (`/Temp/FaceParallaxUndoStack` transient package) and wraps the edit in `GEditor->BeginTransaction`/`Modify()`. Undo/Redo pop the stack, copy the whole preset back (`RestoreFromBackup` — a full copy-back, never a delta), and refresh; a new mutation clears the redo branch. Toolbar Undo/Redo buttons drive it; the old single-slot snapshot survived as **Backup Point**/**Restore Backup** and does not touch the undo stack (`bIsRestoringUndo` guards restores). Mirror tests: `TestUndoStackSemantics`, `TestUndoRedoClearsOnNewMutation`, `TestUndoPreservesUntouchedViews`, `TestUndoStackCap`
...
- **Top-level tab bar + rail regroup (Phase B)** — the old single-char icon column is replaced by a labeled 6-group tab row (`FPLayout::TopTabs`: TT-Tab0..5 + TT-Spacer, widths 82/34/104/122/82/60, `TabBarHeight` 26, `AccentBlue` active highlight): **View & Layer** (Layers + Status Detail + All Layers), **Art** (Quick Actions, Cross-View Transform, Import, Outline→Depth, Bulk Assign, Assign Ops), **Animated Variants** (Viseme Frames + Hull Review), **Nested Elements & Pins** (Nested Art/Pins), **Camera/Preview** (Camera Follow, Camera, Blend Preview), **Advanced** (Config, Param Reference, Param Bindings, Edge Analysis, Depth Debug, Problems, Tag Validator, Material Cross-Reference). Param Reference + Param Bindings were re-homed from Nested & Pins into Advanced (review grouping: params live with the diagnostics); the section registry (`FPLayout::RailSectionTitles`) mirrors the move. Every rail keeps its chip row + section registry; the tab bar sets `ActiveRailIndex` exactly like the old icons, so chips, search jumps, and `SetActiveRailIndex` all stay in sync
...
- **Pin manager (Phase E)** — the Nested Art / Pins section has an **[Elements] / [Pins]** pane switcher (`SetNestedPaneMode`); the Pins pane (`RebuildPinManager`) is the central pin overview the review asked for: one row per pinned item (whole-layer pin, every pinned element, pinned children), each with a visibility toggle, jump-to-element (selects the element and flips back to its controls), and an Unpin button; a Copy row duplicates the selected pin to another element (pin data copied via `FFacePin3D`, undo-scoped). Rows are counted by `FPLayout::FPPinnedRowCount`
- **Pin controls (advanced)** — element `<` `>` stepper (no more hardcoded element 0), live Pinned checkbox, Pin X/Y/Z sliders with numeric readouts, plus view-angle rotation controls: "Rotate w/ view angle" checkbox and Min Rot / Max Rot / Sens sliders; all refreshed by `RefreshPinControls()`; pins are also draggable directly on the canvas (gizmo pin handle)
...
- **Quick Actions** — rail-local batch ops: Duplicate Front→This, Fill Missing Views (copies the active slot's art to views that have no albedo yet); the canonical batch actions (Auto-Fit All, Sync All→All, Clear All Overrides) live only in the pinned strip (P21)
...
- **Toolbar search** — the filter box moved to the toolbar so it applies across all 7 property panels (right pane + 6 rails)
- **Rail accessibility (Phase 4b)** — the big-scroll remediation follow-up, all mirrored by `TestAccessibilityMirrors` (`FPLayout::RailSectionTitles` / `FindRailSectionByTitle` / `ConfigSummary` / `VisemeSummary` / `RailWidthAfterDrag` / `QuickActionLabels`):
  - **Progressive disclosure below section level** — the Advanced rail's 8-checkbox Config section and the Animated rail's Viseme Frames grid each collapse into a one-line summary ("K of 8 on" / "N viseme rows") behind a clickable `SFaceDisclosure` header; summaries update live from `RefreshConfigCheckboxes` / `RebuildVisemeGrid`
  - **Persistent quick-actions bar** — Import Art…, Sync All→All, Auto-Fit All, Clear All Overrides sit in a full-width pinned strip row between the zone diagram and the main row — never inside a scroll viewport (P21 `PinnedActionsNeverInScroll`: the manifest's `PinnedStrip` node holds exactly these four, flagged `bPinnedAction`; the Python validator flags any canonical label built in a rail or scrolled panel). Button set mirrors `FPLayout::QuickActionLabels()`
   - **Cross-rail search jump** — pressing Enter in the toolbar search runs `OnRailSearchCommitted`, finds the first section title containing the query across all 6 rails, switches rail, expands the accordion section, and scrolls it into view (`SScrollBox::ScrollDescendantIntoView`); a "no match" status message is shown otherwise
   - **Drag-resize rail** — `SFaceRailResizer` handle between the rail column and canvas: drag updates `RailWidthPx` live (clamped to `FPLayout::ClampRailWidth` 180–360 via `SetRailWidthLive`, no rebuild during the drag; commit rebuilds so the Advanced rail-width spinbox stays in sync)
...
- **Central-canvas redesign (schematic default view + interactivity + front/base/back yaw rules)** — the canvas now shows per-part glyphs for every part whose mapped layer has NO assigned art, so the full part set is visible before any texture exists:
  ...
  - **Art replaces the outline** — `RefreshSchematic` drops the glyph of any part whose mapped layer has Front albedo; remaining glyphs are transformed by the layer's effective transform in the active view (`FPHotspotTransformPoint` chain), so the schematic hugs the same positions the assigned art will paint; unmapped parts keep the template pose
  - **The canvas is fully clickable (Phase 0)** — Slate routes mouse events to exactly one topmost hit-test-visible leaf plus its ancestors, so the old overlay stack had dead clicks (the gizmo SBox swallowed everything whenever a layer was selected, and nothing below it was ever reachable). Now the gizmo is **paint-only** (`EVisibility::SelfHitTestInvisible` on the SBox and the leaf), and `SFaceHotspotLayer` — the topmost interactive widget — is THE click router with one fixed order: **pin-drag (pin mode, moved out of the gizmo) → named region → schematic glyph → layer-art quad → miss** (misses are swallowed so clicks are never dead). Hover and the Hand cursor are forwarded to `SFaceSchematicLayer` lens- and filter-aware, so what you see is exactly what you can click
  - **Click-to-assign** — clicking a glyph (`HandleSchematicPartClick`) resolves the part to its layer (derivation → alias) and selects it; empty layers open the Import Folder Wizard preselected on that part; layers with art are selected for review only (`HandleHotspotClick` for regions)
  ...
  - **Filter row (Phase 3)** — under the canvas mode row (toggled by the Filter checkbox): a depth radio (All/Front/Base/Back), one toggle chip per base-preset layer (10 chips colored amber/grey/cyan by depth class), the **Focus** zoom, and **Clear**. The row is the pure mirror of `FPSchematicFilterAllows` (empty layer filter = all layers; depth radio 1/2/3; AND semantics) — the math tests pin the mirror
  ...
  - **Depth overlay checkbox** — the canvas mode row's **Depth** toggle (`ToggleDepthOverlay`/`BuildDepthOverlay`) composites the selected layer's depth map over the live preview at 55% opacity
...
- **Canvas click-to-select + selection outline (Phase C)** — beyond the 13 hotspots, a plain left click on the preview hit-tests every layer's UV quad (`FPLayout::FLayerQuadFromTransform` — the same master-material chain the hotspot outlines use) and selects the topmost layer whose quad contains the point (`FPHitTopmostQuad`: draw order = layer list order, last = on top); right-click or ctrl+click cycles through the overlapping layers (`FPCycleQuadHit`). The selected layer's quad is outlined in AccentBlue at 2px, **always visible** (not hover-only), and both the outline and the hit-test are rebuilt from the active view state's stored transforms on every `RefreshUI` — so selection hugs the art in Profile/Back/¾/Top/Bottom too (the cross-view outline constraint)
- **Unified inspect mode (Phase C)** — the canvas's segmented row is now a single 5-mode switcher: **Textured / Outline / Depth / Wireframe / Heatmap** (replaces the old Textured/Depth/Wireframe/Split display row; the legacy Split combo still works via the Config checks). The Advanced rail Config checks (Show Textures / Depth Mesh / Wireframe / Outline overlay / Color by Depth) stay the single source of truth: the row applies canonical combos (`SetInspectMode` → `FPLayout::InspectComboForMode`), and the highlight re-derives from the five toggles on refresh (`FPLayout::DeriveInspectMode`; any other combo — e.g. textures+depth — shows no highlight)
...
- **Undo/Redo toolbar buttons** — Undo/Redo sit in the toolbar next to Save (buttons 2/3, `TB-Undo`/`TB-Redo` in the Phase H manifest); the old single-slot "Snapshot" pair was relabeled **Backup Point**/**Restore Backup** with an explanatory tooltip clarifying it is a manual safety copy, not multi-step undo; Clear State/Clear All require a confirming second click; "Log: ON/OFF" collapses the diagnostic log
...
- **Dev tools relocated (Phase A)** — Tag Validator and Material Cross-Reference moved out of the bottom bar into the Advanced rail as accordion sections (`Sec-TagValidator`/`Sec-MatCrossRef` in the Phase H manifest); the bottom bar now holds only workflow actions
...
| `SetActiveRailIndex(Index)` | Switches the left rail panel (0 View & Layer, 1 Art, 2 Animated Variants, 3 Nested Elements & Pins, 4 Camera/Preview, 5 Advanced) |
...
| `SelectCanvasLayerAt(UV)` / `CycleCanvasLayerAt(UV)` | Phase C: canvas layer-art pick — select the topmost quad hit, or right/ctrl-click cycle through overlapping layers |
...
| `RegisterAccordionSections(RailIdx, Accordion)` | Bulk-registers every accordion section in visual order (Art 2, Animated 2, Nested & Pins 3, Advanced 6) |
...
| `HandleHotspotClick(RegionName)` | Canvas/parts-strip pick: resolves the region to a layer, selects it, opens the Art rail, and opens the Import Folder Wizard preselected on that part |
| `HandleSchematicPartClick(PartName)` | Canvas glyph pick (Phase 0 router): resolves part → layer (derivation → alias), selects it; empty layers open the Import Folder Wizard |
...
FaceParallaxLayoutSpec.h                 — Phase H layout manifest + P1..P21 design validator
...
Tests/
  ParallaxMathTests.cpp                  — 1550 standalone C++ tests
