...
| `Tests/ParallaxMathTests.cpp` | Math tests | Standalone C++17 (no UE dep) — 2085 tests covering state machine, transforms, blink/expression/viseme, swoosh, parameters, nested art + jiggle, 3D pin projection + view-angle rotation, batch ops, zone multipliers, per-view visual hull, Phase B–H mirror suites + Phase H layout-design contract, part schematic + yaw rules + coverage/aliases + hair system + canvas filters + midpoint jiggle ramp + edge-map group mirrors, the hybrid smooth-turn orientation contract (`TestPhase2Orientation`), the up/down pitch scrub contract (`TestPhaseCUpDownScrub`), the authored-pose morph suite (`TestAuthoredOrientation`), anchor-class mirrors (`TestAnchorClass`), the per-state visibility + Z-order contract (`TestPhase3Visibility`), the silhouette-delta crossfade/swoosh contract (`TestPhase4SilhouetteDelta`), and the authored-pose validation with negative controls (`TestPhase6PoseValidation`) |
...
- **Central-canvas redesign (schematic default view + interactivity + front/base/back yaw rules)** — the canvas shows the 17-part schematic as THE map (P1 one-map); glyphs are transformed by the layer's effective transform so the map hugs the rendered head:
  ...
  - **Smooth-turn orientation (Phase B rework, canonical spec)** — the placeholder is now **billboarded**: every part is a flat card that always faces the camera (the surface normal never turns edge-on / never paper-thins — real 2D art), and the turn is faked by **sliding five Z-depth planes** against each other via camera-translation parallax (`FPZDepth` 1 Nose/Front Bangs → 5 Neck/Back Hair; `FPYawSlidePeak`/`FPYawSlideAt`: closest Z slides furthest, the Z-5 backdrop never slides), with a slide-then-flip at each exact 45/90/135/180 yaw state center (authored 2D layout ramps, `FPRampEval` smoothstep — per-view 2D art stays in sync). Yaw: silhouettes stay full width at 0 AND 180, the far-side pair (eye/ear/cheek) folds to zero by yaw 90 and STAYS folded through the back, features walk-behind fade 1.0→0 across 112.5–135, the nose darts toward the far edge, and mirror symmetry + left-half mirroring are preserved. Pitch: `FPOrientationPitchScale` squashes vertically toward the extremes and `FPOrientationVerticalShift` is the encroach/counter contract — features + hair ENCROACH down at Top / up at Bottom, the ears + V-chin COUNTER-translate (top view tucks the chin), the face base stays near-static. The old hybrid helpers are removed. Pinned by `TestPhase2Orientation`. **Authored silhouette poses** complete the placeholder for the four silhouette parts: `FPSchematicAuthoredPoseTable` carries authored key poses (0/45/90/135/180 + Top/Bottom) for Head/Bangs/Hair/BackHair, `FPOrientationOutline` branches to `FPOrientationAuthoredMorph` (smoothstep between the bracketing yaw anchors, pitch blend weighted by yaw-fade, negative-yaw mirror) for them while every feature keeps the formula; the BackHair profile pose now TRAILS the skull. Anchor classes (`FPSchematicAnchorClass`: silhouettes + ears AnchorCritical, features BridgeSafe), per-state visibility + Z-order (`FPSchematicLayerVisibleInState`/`FPSchematicLayerVisibleInTag` — the runtime hides the walk-behind feature cards via `RefreshLayerVisibilityForState` so art never edge-peeks), the silhouette-delta crossfade/swoosh (`FPSilhouetteDelta`/`FPSchematicTransitionBlendRate`/`FPSchematicShouldSwoosh` — the runtime swoosh trigger and blend tick both consult them), and the pose validator (`FPOutlineIsValidClosedRing`/`FPSchematicValidateAllAuthoredPoses`, 41% back-change gate) round out the contract
  - **Up/down view scrub (Phase C)** — a dedicated vertical **Pitch view** strip mounted **under the 360° yaw zone diagram** is the up/down counterpart of the yaw zone scrub: drag up lifts the head toward **Top**, drag down lowers it toward **Bottom**; the relative pixel drag is fed through the pure `FPLayout::FPZoneScrubPitchAfterDrag` contract (full strip = 180°, clamped `[-90,90]`, NO wrap) and drives `SetOrbitPitch` + `ActiveViewState` live (`DetermineStateFromAngles` parks at the Top/Bottom art states past their thresholds — both are real art states with vertical parallax). Release snaps the pitch to the canonical state center so **Top parks at +90 / Bottom at −90**. `RefreshSchematic` prefers the scrub yaw/pitch while either strip is dragging (`bScrubbing = bZoneScrubbing || bPitchScrubbing`). Pinned by `TestPhaseCUpDownScrub` + the vertical-parallax pins in `TestPhase2Orientation`
